...
//get particular card details
cards.addEventListener("click",(ele)=>{
  if(ele.target.classList.contains("buy-btn")){
    let clicked_btn=ele.target.dataset.id
    let product=Data.find((pro)=>pro.id==clicked_btn)
    console.log(product)
    window.location.href=`./singlepage.html?id=${clicked_btn}`
  }
})
...
for(let ele of filtered_data){
    ...
    text.innerHTML=`<h4>Title:${ele.title}</h4><h5>Price:\u20B9${ele.price}</h5><button class="buy-btn" data-id="${ele.id}">Buy</button> <button class="add-btn" data-id="${ele.id}">Add to cart</button>`
...
search_bar.addEventListener("input",()=>{
   let user_entered_data=search_bar.value.toLowerCase().trim()
   if(user_entered_data!==""){
    let filter_data=Data.filter((ele)=>ele.title.trim().toLowerCase().includes(user_entered_data))
    if(filter_data.length>0){  
    cards.textContent=""
    display(filter_data)
    }else{
      cards.textContent=""
      let no_match= document.createElement("h3")
       no_match.textContent="No data found..."
       no_match.style.color="red"
       cards.appendChild(no_match)
    }
...
cart_symbol.classList.add("cart-icon-style")
